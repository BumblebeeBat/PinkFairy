-module(sign_tx).
-export([doit/6, make_tx/6, test/0]).
%test/0, doit/8, htoi/1, itoh/1, winner/5, sign/1, winners/1, acc/1, secret_hash/1, winners_length/1, number/1, prev_hash/1, repeat/2, finality_ancestor/1]).
-record(sign_tx, {acc = 0, nonce = 0, fee = 0, winners = [], recent_hash = 0, block_height = 0, governance = [], oracle_decisions = []}).

make_tx(ID, BlockHash, Fee, Governance, OracleDecisions, Accounts) ->
    {_, Acc, Proof} = trie:get(ID, Accounts, accounts),
    Winners = [],
    Height = 0,
    Tx = #sign_tx{acc = ID, nonce = account:nonce(Acc) + 1, fee = Fee, winners = Winners, recent_hash = BlockHash, block_height = Height, governance = Governance, oracle_decisions = OracleDecisions},
    {Tx, [Proof]}.

doit(Tx, _,_,_Variables, Height, Return) ->
    _Vars = Tx#sign_tx.governance,
    %compare vars to current values, and decide if we are pushing up, or down. record 1 or -1 into a new vars.
    %if we want to move a very small amount, then probabilistically switch the number to 0 based on the randomness from the random number generator.
    VU = ok,
    RevealSecret = 0,%get this from the gen_server that holds the secret chain
    NewAccount = account:new_update(Tx#sign_tx.acc, -Tx#sign_tx.fee, [Tx#sign_tx.nonce], Height, [RevealSecret]),
    Return ! {[], [NewAccount], [VU]}. 
...
%winners(MyPower, TotalPower, Entropy, Addr) ->
%    winners(MyPower, TotalPower, Entropy, Addr, 0, constants:chances_per_address(), [])%.
%winners(_, _, _, _, J, Limit, Out) when J > Limit -> Out;
%winners(MyPower, TotalPower, Entropy, Addr, J, Limit, Out) ->
%    B = winner(MyPower, TotalPower, Entropy, Addr, J),
%    NOut = if
%        B -> [J|Out];
%        true -> Out
%    end,
%%    winners(MyPower, TotalPower, Entropy, Addr, J+1, Limit, NOut).
%sign(Id) ->
%    %Id = keys:id(),
...
%%    W = winners(MyPower, TotalPower, Entropy, accounts:addr(Acc)),
...
%winner(MyBonds, TotalBonds, Seed, Addr, J) ->
...
%    all_winners(MyPower, block_tree:block_power(Block), block_tree:block_entropy(Block), accounts:addr(Acc), Tx#sign_tx.winners),
